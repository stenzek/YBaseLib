...
public:
    ...
    void ClearErrorState() { m_errorState = false; m_pStream->ClearErrorState(); }
    ...
    void WriteBool(const bool b) { byte bb = b ? 1 : 0; InternalWriteBytes(&bb, sizeof(bb)); }
    ...
    bool SafeWriteBool(const bool b) { byte bb = b ? 1 : 0; return SafeInternalWriteBytes(&bb, sizeof(bb)); }
    ...
    template<typename T> void WriteType(const T *v) { InternalWriteBytes(v, sizeof(T)); }
    template<typename T> bool SafeWriteType(const T *v) { return SafeInternalWriteBytes(v, sizeof(T)); }

    // stream operators for reading to local variables
    // at the moment doing this provides no speed boost over the readX functions.
    inline BinaryWriter &operator<<(const int8 src) { WriteInt8(src); return *this; }
    inline BinaryWriter &operator<<(const uint8 src) { WriteUInt8(src); return *this; }
    inline BinaryWriter &operator<<(const int16 src) { WriteInt16(src); return *this; }
    inline BinaryWriter &operator<<(const uint16 src) { WriteUInt16(src); return *this; }
    inline BinaryWriter &operator<<(const int32 src) { WriteInt32(src); return *this; }
    inline BinaryWriter &operator<<(const uint32 src) { WriteUInt32(src); return *this; }
    inline BinaryWriter &operator<<(const int64 &src) { WriteInt64(src); return *this; }
    inline BinaryWriter &operator<<(const uint64 &src) { WriteUInt64(src); return *this; }
    inline BinaryWriter &operator<<(const float &src) { WriteFloat(src); return *this; }
    inline BinaryWriter &operator<<(const double &src) { WriteDouble(src); return *this; }
    inline BinaryWriter &operator<<(const String &src) { WriteCString(src); return *this; }
    inline BinaryWriter &operator<<(const char *src) { WriteCString(src); return *this; }
