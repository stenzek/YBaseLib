...
void Y_OnAssertFailed(const char *szMessage, const char *szFunction, const char *szFile, uint32 uLine);
void Y_OnPanicReached(const char *szMessage, const char *szFunction, const char *szFile, uint32 uLine);

#define Assert(expr) if (!(expr)) { Y_OnAssertFailed("Assertion failed: '" #expr "'", __FUNCTION__, __FILE__, __LINE__); }
#define AssertMsg(expr, msg) if (!(expr)) { Y_OnAssertFailed("Assertion failed: '" msg "'", __FUNCTION__, __FILE__, __LINE__); }
...
#define DefaultCaseIsUnreachable() default: UnreachableCode(); break;
