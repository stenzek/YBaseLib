...
typedef MemArray<FILESYSTEM_FIND_DATA> FindResultsArray;
...
ChangeNotifier *CreateChangeNotifier(const char *path, bool recursiveWatch);

// appends a path string to the current path string. optionally canonicalizes it.
void AppendPath(char *Path, uint32 cbPath, const char *NewPath);
void AppendPath(String &Path, const char *NewPath);

// canonicalize a path string (i.e. replace .. with actual folder name, etc), if OS path is used, on windows, the separators will be \, otherwise /
void CanonicalizePath(char *Destination, uint32 cbDestination, const char *Path, bool OSPath = true);
...
void BuildOSPath(char *Destination, uint32 cbDestination, const char *Path);
...
void BuildPathRelativeToFile(char *Destination, uint32 cbDestination, const char *CurrentFileName, const char *NewFileName, bool OSPath = true, bool Canonicalize = true);
void BuildPathRelativeToFile(String &Destination, const char *CurrentFileName, const char *NewFileName, bool OSPath = true, bool Canonicalize = true);

// sanitizes a filename for use in a filesystem.
void SanitizeFileName(char *Destination, uint32 cbDestination, const char *FileName, bool StripSlashes = true);
...
// search for files
bool FindFiles(const char *Path, const char *Pattern, uint32 Flags, FindResultsArray *pResults);    
