...
{
    Log_ErrorPrintf("FileSystem::CreateChangeNotifier(%s): Unsupported on HTML5.", path);
    return nullptr;
}
...
//            outData.Attributes |= FILESYSTEM_FILE_ATTRIBUTE_READ_ONLY;
        ...
        {
            if (ParentPath != NULL)
                Y_snprintf(outData.FileName, countof(outData.FileName), "%s/%s/%s/%s", OriginPath, ParentPath, Path, pDirEnt->d_name);
            else if (Path != NULL)
