...
uint32 StringConverter::StringToColor(const char *Source)
{
    ...
    case 3:
        outColor = (uint32)sourceBytes[0] | (uint32)sourceBytes[1] << 8 | (uint32)sourceBytes[2] << 16 | (uint32)0xff << 24;
        break;

    case 4:
        outColor = (uint32)sourceBytes[0] | (uint32)sourceBytes[1] << 8 | (uint32)sourceBytes[2] << 16 | (uint32)sourceBytes[3] << 24;
        break;
...
void StringConverter::ColorToString(String &Destination, const uint32 Value)
{
    ...
    if (((Value >> 24) & 0xff) == 0xff)
        ...
        Y_snprintf(str, countof(str), "#%02x%02x%02x%02x", (Value & 0xff), ((Value >> 8) & 0xff), ((Value >> 16) & 0xff), ((Value >> 24) & 0xff));
