...
Subprocess::Subprocess(HANDLE hChildProcess, DWORD childProcessID, HANDLE hQuitEvent, HANDLE hReadPipe, HANDLE hWritePipe)
    : m_hChildProcess(hChildProcess), m_childProcessID(childProcessID), m_hQuitEvent(hQuitEvent), 
      m_hReadPipe(hReadPipe), m_hWritePipe(hWritePipe), m_pChildConnection(nullptr)
{
...
Subprocess *Subprocess::Create(const char *executableFileName, const char *parameters /* = NULL */, bool openChannel /* = true */, bool newConsole /* = true */, bool terminateOnParentLost /* = true */)
{
    ...
    if (openChannel)
    {
        ...
        uint32 targetEnvironmentSize = 0;
        {
            ...
            {
                if (sourceStrings[sourceStringsSize] == '\0' &&
                    sourceStrings[sourceStringsSize + 1] == '\0')
                {
                    // Found the end
                    sourceStringsSize++;
                    break;
        ...
        if (!CreateProcessA(executableFileName, NULL, NULL, NULL, TRUE, creationFlags, targetEnvironmentString, NULL, &si, &pi))
        {
...
Subprocess::Connection *Subprocess::ConnectToChild(int32 connectionTimeout /*= -1*/)
{
    ...
    if (!WriteFile(m_hWritePipe, &PARENT_READY_MESSAGE_VALUE, sizeof(PARENT_READY_MESSAGE_VALUE), &transferred, NULL) || transferred != sizeof(PARENT_READY_MESSAGE_VALUE))
    {
    ...
    if (!ReadFile(m_hReadPipe, &readyMessage, sizeof(readyMessage), &transferred, NULL) || transferred != sizeof(readyMessage) || readyMessage != CHILD_READY_MESSAGE_VALUE)
    {
...
Subprocess::Connection *Subprocess::ConnectToParent(int32 connectionTimeout /*= -1*/)
{
    ...
    if (!ReadFile(hReadPipe, &readyMessage, sizeof(readyMessage), &transferred, NULL) || transferred != sizeof(readyMessage) || readyMessage != PARENT_READY_MESSAGE_VALUE)
    {
    ...
    if (!WriteFile(hWritePipe, &CHILD_READY_MESSAGE_VALUE, sizeof(CHILD_READY_MESSAGE_VALUE), &transferred, NULL) || transferred != sizeof(CHILD_READY_MESSAGE_VALUE))
    {
...
Subprocess::Connection::Connection(HANDLE hTargetProcess, HANDLE hQuitEvent, HANDLE hReadPipe, HANDLE hWritePipe, bool isParentConnection, bool terminateOnParentLost)
    : m_hTargetProcess(hTargetProcess), m_hQuitEvent(hQuitEvent),
      m_hReadPipe(hReadPipe), m_hWritePipe(hWritePipe),
