...
{
public:
    ZipArchiveStreamedReadByteStream(ZipArchive *pZipArchive, ByteStream *pArchiveStream, uint64 baseOffset, ZIP_ARCHIVE_LOCAL_FILE_HEADER *pLocalFileHeader)
        : m_pZipArchive(pZipArchive),
          m_pArchiveStream(pArchiveStream),
          m_baseOffset(baseOffset),
          m_currentFileOffset(baseOffset),
          m_inBufferBytes(0),
          m_inBufferPosition(0),
          m_currentDecompressedOffset(0),
          m_currentCRC32(0)
    {
    ...
    bool FillInputBuffer()
    {
        ...
        uint32 readSize = (uint32)Min(m_localFileHeader.CompressedSize - (m_currentFileOffset - m_baseOffset), (uint64)sizeof(m_pInBuffer));
    ...
    virtual uint32 Read(void *pDestination, uint32 ByteCount)
    {
        ...
        case 0:
            {
                ...
                if (m_currentDecompressedOffset == m_localFileHeader.DecompressedSize && m_currentCRC32 != m_localFileHeader.CRC32)
                {
                    Log_ErrorPrintf("ZipArchiveStreamedReadByteStream::Read: CRC mismatch: %u / %u", m_currentCRC32, m_localFileHeader.CRC32);
                    m_errorState = true;
                    return 0;
                }
        ...
        default:
            {
                Log_ErrorPrintf("ZipArchiveStreamedReadByteStream::Read: Unknown compression method: %u", (uint32)m_localFileHeader.CompressionMethod);
                m_errorState = true;
                return 0;
            }
            break;
    ...
    virtual bool WriteByte(byte SourceByte)
    {
        return false;
    }
    ...
    virtual bool Write2(const void *pSource, uint32 ByteCount, uint32 *pNumberOfBytesWritten = nullptr)
    {
        return false;
    }
    ...
    {
        return m_currentDecompressedOffset;
    }

    virtual uint64 GetSize() const
    {
        return m_localFileHeader.DecompressedSize;
    }
...
public:
    ...
    ~ZipArchiveBufferedReadByteStream()
    {
        m_pZipArchive->m_nOpenBufferedReads--;
    }
    ...
    virtual uint64 GetPosition() const
    {
        return (uint64)m_position;
    }

    virtual uint64 GetSize() const
    {
        return (uint64)m_size;
    }
...
{
public:
    ZipArchiveStreamedWriteByteStream(ZipArchive *pZipArchive, ByteStream *pArchiveStream, ZipArchive::FileEntry *pFileEntry, uint64 baseOffset, uint32 compressionMethod, uint32 compressionLevel)
        : m_pZipArchive(pZipArchive),
          m_pArchiveStream(pArchiveStream),
          m_pFileEntry(pFileEntry),
          m_baseOffset(baseOffset),
          m_currentFileOffset(baseOffset),
          m_currentCompressedSize(0),
          m_currentDecompressedSize(0),
          m_outBufferBytes(0),
          m_currentCRC32(0),
    ...
    virtual bool Read2(void *pDestination, uint32 ByteCount, uint32 *pNumberOfBytesRead = NULL)
    {
        return false;
    }
    ...
    virtual bool WriteByte(byte SourceByte)
    {
        return (Write(&SourceByte, sizeof(byte)) == sizeof(byte));
    }
    ...
    virtual uint64 GetPosition() const
    {
        return m_currentDecompressedSize;
    }
...
{
public:
    ZipArchiveBufferedWriteByteStream(ZipArchive *pZipArchive, ByteStream *pWriteStream, ZipArchive::FileEntry *pFileEntry, uint32 compressionMethod, uint32 compressionLevel)
    {
...
ZipArchive::ZipArchive(ByteStream *pReadStream, ByteStream *pWriteStream)
    : m_pReadStream(pReadStream),
      m_pWriteStream(pWriteStream),
      m_nOpenStreamedReads(0),
      m_nOpenStreamedWrites(0),
      m_nOpenBufferedReads(0),
      m_nOpenBufferedWrites(0)
{
    if (pReadStream != NULL)
        pReadStream->AddRef();
...
ByteStream *ZipArchive::OpenFile(const char *filename, uint32 openMode, uint32 compressionLevel /* = 6 */)
{
    ...
    if (openMode & BYTESTREAM_OPEN_READ)
    {
        ...
        if (!binaryReader.SafeReadUInt32(&localFileHeader.Signature) ||
            ...
            !binaryReader.SafeReadUInt32(&localFileHeader.CRC32) ||
            !binaryReader.SafeReadUInt32(&compressedSize32) ||
        ...
        {
            // create stream
            ZipArchiveStreamedReadByteStream *pReaderStream = new ZipArchiveStreamedReadByteStream(this, pStreamToReadFrom, pStreamToReadFrom->GetPosition(), &localFileHeader);
    ...
    else if (openMode & BYTESTREAM_OPEN_WRITE)
    {
        ...
        if (streamed)
        {
            ZipArchiveStreamedWriteByteStream *pWriterStream = new ZipArchiveStreamedWriteByteStream(this, m_pWriteStream, pMember->Value, m_pWriteStream->GetPosition(), compressionMethod, compressionLevel);
        ...
        {
            ZipArchiveBufferedWriteByteStream *pWriterStream = new ZipArchiveBufferedWriteByteStream(this, m_pWriteStream, pMember->Value, compressionMethod, compressionLevel);
            return pWriterStream;
        }
    }
...
bool ZipArchive::CopyFile(ZipArchive *pOtherArchive, const char *filename)
{
    ...
    // write local header
    {
        ...
        if (localFileHeader.Flags & 8)
        {
            ...
            // pull from central directory, and strip the streamed flag from it, since we're not going to append the post-header
...
bool ZipArchive::ParseZip()
{
    ...
    if (!binaryReader.SafeReadUInt16(&diskNumber) ||
        !binaryReader.SafeReadUInt16(&centralDirectoryDisk) ||
        !binaryReader.SafeReadUInt16(&centralDirectoryRecordsOnThisDisk) ||
        !binaryReader.SafeReadUInt16(&centralDirectoryTotalRecords) ||
        !binaryReader.SafeReadUInt32(&centralDirectorySize) ||
        !binaryReader.SafeReadUInt32(&centralDirectoryOffset) ||
    ...
    for (uint16 i = 0; i < centralDirectoryTotalRecords; i++)
    {
        ...
        if (!binaryReader.SafeReadUInt32(&fileSignature) ||
            !binaryReader.SafeReadUInt16(&versionMadeBy) ||
            !binaryReader.SafeReadUInt16(&versionExtract) ||
            !binaryReader.SafeReadUInt16(&bitFlags) ||
            !binaryReader.SafeReadUInt16(&compressionMethod) ||
            !binaryReader.SafeReadUInt32(&modificationTimestamp) ||
            !binaryReader.SafeReadUInt32(&crc) ||
            !binaryReader.SafeReadUInt32(&compressedSize) ||
            !binaryReader.SafeReadUInt32(&uncompressedSize) ||
        ...
        if (((uint32)extraFieldLength + (uint32)fileCommentLength) > 0 && !m_pReadStream->SeekRelative((uint32)extraFieldLength + (uint32)fileCommentLength))
            return false;
...
bool ZipArchive::CommitChanges()
{
    ...
    // any files that are located in the read stream, but not in the write stream, now need to be written to the write stream
    ...
    // write the central directory to the write stream
    {
        ...
        if (!binaryWriter.SafeWriteUInt32(ZIP_ARCHIVE_END_OF_CENTRAL_DIRECTORY_SIGNATURE) ||                 // Signature
            ...
            !binaryWriter.SafeWriteUInt16((uint16)nCentralDirectoryEntries) ||                               // Central directory record count on this disk
...
bool ZipArchive::FindFiles(const char *path, const char *pattern, uint32 flags, FileSystem::FindResultsArray *pResults) const
{
    // has a path
    if (path[0] == '\0')
        return false;
...
uint32 ZipArchive::FindFilesInTable(const FileHashTable &fileTable, const char *path, const char *pattern, uint32 flags, FileSystem::FindResultsArray *pResults) const
{
