...
static String::StringData *StringDataAllocate(uint32 allocSize)
{
    ...
    String::StringData *pStringData = reinterpret_cast<String::StringData *>(Y_malloc(sizeof(String::StringData) + allocSize));
...
void String::InsertString(int32 offset, const char *appendStr, uint32 appendStrLength)
{
    ...
    if (charactersAfterOffset > 0)
        Y_memmove(m_pStringData->pBuffer + offset + appendStrLength, m_pStringData->pBuffer + offset, charactersAfterOffset);
...
bool String::StartsWith(const String &compareString, bool caseSensitive /*= true*/) const
{
    ...
    return (caseSensitive) ? (Y_strncmp(compareString.m_pStringData->pBuffer, m_pStringData->pBuffer, compareStringLength) == 0) :
...
bool String::EndsWith(const String &compareString, bool caseSensitive /*= true*/) const
{
    ...
    uint32 startOffset = m_pStringData->StringLength - compareStringLength;
    return (caseSensitive) ? (Y_strncmp(compareString.m_pStringData->pBuffer, m_pStringData->pBuffer + startOffset, compareStringLength) == 0) :
                             (Y_strnicmp(compareString.m_pStringData->pBuffer, m_pStringData->pBuffer + startOffset, compareStringLength) == 0);
}
...
void String::Resize(uint32 newSize, char fillerCharacter /* = ' ' */, bool skrinkIfSmaller /* = false */)
{
    ...
    if (StringDataIsShared(m_pStringData) || m_pStringData->ReadOnly || newSize >= m_pStringData->BufferSize)
    {
        ...
        if (m_pStringData->StringLength < newSize)
            Y_memset(m_pStringData->pBuffer + m_pStringData->StringLength, (byte)fillerCharacter, m_pStringData->BufferSize - m_pStringData->StringLength - 1);
...
#if Y_BUILD_CONFIG_DEBUG
        Y_memzero(m_pStringData->pBuffer + m_pStringData->StringLength, m_pStringData->BufferSize - m_pStringData->StringLength);
#else
        m_pStringData->pBuffer[m_pStringData->StringLength] = 0;
#endif
    }
